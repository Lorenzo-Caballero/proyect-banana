...
> **Hecho** (commit `0aad330` del bot): `conc` quedó en 3. Si los challenges
> siguen, el siguiente paso es 2.
...
> **Hecho** (commit `0aad330` del bot): `alta_api.es_challenge(cuerpo)` es la
> función única, y la usan el depósito (que ahora decide por el cuerpo y
> devuelve `reintentar` ante un challenge — la cola de GOLDPAW ya lo soporta
> con reintentos acotados) y el fast-path de altas (que reintenta hasta 3
> veces en vez de caer al formulario, que cruza el mismo WAF).
...
> **Ojo, es un problema DISTINTO del de la réplica.** `PARA-FAUNO-juegos.md`
> documenta el otro: desde `ganamoscrm.online` el lanzamiento manda
> `x-actual-domain` con el dominio del navegador y el proveedor puede
> rechazarlo por lista blanca. Ese rompe la PRIMERA apertura desde la réplica;
> este (el token quemado) rompe la SEGUNDA apertura en cualquier dominio.
...
1. ~~**`conc` de 6 a 2–3** en el fast-path de altas.~~ **Hecho** (`0aad330`,
   quedó en 3). Falta desplegar con `deploy-bot.sh`.
2. ~~**Detectar el challenge por el cuerpo y reintentar**, en el depósito y en
   el alta.~~ **Hecho** (`0aad330`). Falta desplegar. Con esto el parche en
   caliente del contenedor queda obsoleto: el código del repo trae las marcas
   `# [goldpaw]` y el cron guardián lo va a ver como "ya parchado".
