...
const CONTEXTO = <<<TXT
Sos el asistente de atencion al cliente de ganamos, un videojuego de guerra de
drones incrementales. Ayudas a los jugadores con dudas y con la CARGA DE FICHAS.
Las "fichas" son la moneda del juego. Ademas existen "bonos" (fichas de regalo).
...
Cuando pida retirar, cobrar o sacar plata, usa retirar_del_juego con la
cantidad. Solo eso: el usuario ya lo sabe el server.
- NO es automatico como la carga. Deja el pedido registrado y lo resuelve un
  AGENTE. Deciselo tal cual: nunca le prometas que en un rato lo tiene.
- Si devuelve 'sin_saldo', decile cuanto tiene y que puede retirar hasta ahi.
...
$TOOLS = [
    ...
    ['type' => 'function', 'function' => [
        ...
        'description' => 'Registra un pedido de RETIRO: sacar saldo del juego. No es automático, '
            . 'lo resuelve un agente. Usala cuando el jugador pide retirar, cobrar o sacar plata. '
            . 'Solo necesita la cantidad; el usuario sale de su sesión.',
        'parameters' => [
            'type' => 'object',
            'properties' => [
                'cantidad' => ['type' => 'integer', 'description' => 'cuantas fichas quiere retirar'],
            ],
            'required' => ['cantidad'],
        ],
    ]],
...
$ejecutarTool = function (string $nombre, array $args) use ($pdo, &$usuarioDetectado, $usuarioCliente, $sesionVerificada): array {
    ...
    return ejecutar_tool($pdo, $nombre, $args, $usuarioCliente, $sesionVerificada);
};
...
echo json_encode(['ok' => true, 'respuesta' => $texto], JSON_UNESCAPED_UNICODE);
...
function ejecutar_tool(PDO $pdo, string $nombre, array $args, string $usuarioSesion = '', bool $sesionVerificada = false): array
{
    if ($nombre === 'cargar_al_juego') {
        ...
        return fichas_pedir_carga($pdo, $usuarioSesion, (int)($args['cantidad'] ?? 0), 'chatbot');
    }
